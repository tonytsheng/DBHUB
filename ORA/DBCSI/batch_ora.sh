...
echo ""
echo ""
echo "AWS DB CSI Batch Data Gathering tool"
echo ""
...
if [ -e batch_ora.properties ]
then
	propfile=batch_ora.properties
fi
...
for ENTRY in `cat batch_ora.properties|grep -v 'ORACLE_HOME'|grep -v '#'|grep -v '^[ \t]*\#'`
do
	dbuser=`echo $ENTRY|awk -F: '{print $1}'`
	pw=`echo $ENTRY|awk -F: '{print $2}'`
	hostname=`echo $ENTRY|awk -F: '{print $3}'`
	port=`echo $ENTRY|awk -F: '{print $4}'`
	servicename=`echo $ENTRY|awk -F: '{print $5}'`
	diagnosticspack=`echo $ENTRY|awk -F: '{print $6}'`
...
do
	...
	then
		...
		echo "$diagpackprefix" | $ORACLE_HOME/bin/sqlplus -S $dbuser/$pw@//$hostname:$port/$servicename @../aws_awr_miner.sql >>zzsqlplus.log 2>&1
	else
		...
		$ORACLE_HOME/bin/sqlplus -S $dbuser/$pw@//$hostname:$port/$servicename @../aws_statspack_miner.sql >>zzsqlplus.log 2>&1
	fi
...
orahome=`grep ORACLE_HOME $propfile|awk -F= '{print $2}'`
if [ -z $orahome ]
then
	echo ""
	echo -n "Enter ORACLE_HOME path : "
	read orahome
fi
...
test_orahome ()
{
test -f $orahome/bin/sqlplus
if [ $? != 0 ]
	then
	echo
	echo "The Oracle home entry is incorrect.  Please retry."
	get_orahome
fi
}
