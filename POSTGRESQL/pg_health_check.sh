...
DBCLASS=`aws rds describe-db-instances --profile ec2 --db-instance-identifier $RDSNAME --region $REGNAME --output json| grep Class| awk '{ print $2 }'|sed 's/"//g' |sed 's/db.//g' |sed 's/,//g'`
NUM2=1048576
NUM3=$((NUMSBRAW*8 / NUM2))
SBNUM=$((NUM3*1024))
INSTCLASS=`aws rds describe-db-instances --profile ec2 --db-instance-identifier $RDSNAME | grep Class| awk '{ print $2 }'|sed 's/"//g' |sed 's/db.//g' |sed 's/,//g'`
TOTALRAM=`aws ec2 describe-instance-types --profile ec2 --instance-types $INSTCLASS | grep SizeInMiB | awk '{ print $2 }'`
